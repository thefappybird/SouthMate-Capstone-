...
app.get("/landing", isAuthenticated, async (req, res) => {
  const user = req.session.user;
  const transactions = await transactionRoute
    ...
    if (data.success) {
      ...
      if (bankUser.balance < cashinAmount) {
        res.json({
          ...
          message: "Insufficient funds from bank",
        });
        return;
      }
  ...
  try {
    ...
    if (data.success) {
      ...
      if (user.balance < cashoutAmount) {
        res.json({
          captchaSuccess: false,
          message: "Insufficient funds from account",
        });
        return;
      }
      const bankUser = await bankRoute.findOne({ bankName: bankOption });
  ...
  try {
    ...
    if (data.success) {
      ...
      if (user.balance < sendAmount) {
        res.json({
          captchaSuccess: false,
          message: "Insufficient funds to send.",
        });
        return;
      }
      // Update the user's balance
  ...
  const mailOptions = {
    ...
    to:
      details.type !== "Student"
        ? details.email
        : [details.email, details.guardianEmail],
